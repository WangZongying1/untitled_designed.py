...
from PyQt5.QtGui import QWheelEvent, QMouseEvent, QTransform, QPaintEvent, QShowEvent, QPainter, QPaintDevice, QPen, \
    QBrush, QFont
    ...
    def process_mouse_event(self, event: QMouseEvent):
        ...
        if event.button() == Qt.MiddleButton:
            ...
            if event.type() == QEvent.MouseButtonPress:
                ...
                return True
            elif event.type() == QEvent.MouseButtonDblClick:  # 鼠标左键双击，获得坐标
                # self.mouse_press_view = QPoint(event.pos())
                position = (self.view.mapToScene(event.pos()))  # 获取坐标
                radius = 4#设置绘出点的直径
                cycle = self.scene.addEllipse(0, 0, radius, radius,QPen(Qt.black), QBrush(Qt.green))

                items_x = position.x()  # 将坐标中的x，y坐标分别提取出来
                items_y = position.y()
                cycle.setPos(items_x-radius/2, items_y-radius/2)
                print('(', items_x, ',', items_y, ')')
                self.L_x.append(items_x)
                self.L_y.append(items_y)
                print('点击次数', len(self.L_x))
                tex = self.scene.addText(str(len(self.L_x)))
                tex.setFont(QFont(self.font().family(), 16))
                tex.setPos(items_x, items_y)
                return True
